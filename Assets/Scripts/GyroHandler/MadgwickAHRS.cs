...

{
    ...
    private float beta;
    private float betaMoving = 0.005f; // Smoother when moving fast
    private float betaStill = 0.1f;    // More correction when still
    public float[] Quaternion { get; private set; }

    public MadgwickAHRS(float samplePeriod, float beta = 0.004f)
    {
        this.samplePeriod = samplePeriod;
        this.beta = beta;
        this.Quaternion = new float[] { 0f, 0f, 0f, 1f }; // Initial orientation
    }

    public void Update(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz)
    {
        if (Mathf.Abs(gx) < 0.1f && Mathf.Abs(gy) < 0.1f && Mathf.Abs(gz) < 0.1f)
        {
            return;
        }
